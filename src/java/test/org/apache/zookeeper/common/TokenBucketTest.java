...
public class TokenBucketTest {
    ...
    public void testNoMoreTokens() {
        ...
        tb.nextRefillTime = System.nanoTime();
        for (int i = 0; i < 10; i++) {
            assertTrue(tb.tryAquire());
        }
        assertFalse(tb.tryAquire());
    ...
    public void testRefill() {
        ...
        tb.nextRefillTime = System.nanoTime() - (2 * tb.refreshPeriodNanos);
        assertTrue(tb.tryAquire());
    ...
    public void testMultipleThreadBurst() throws Exception {
        ...
        tb.refreshPeriodNanos = TimeUnit.SECONDS.toNanos(30);
    ...
    public void testRateLimiterFactory() throws InterruptedException {
        RateLimiter limiter = RateLimiter.Factory.create(TokenBucket.class.getName(), 1, 1);
    ...
    public void testBypass() {
        RateLimiter limiter = RateLimiter.Factory.create(TokenBucket.class.getName(), 1,
                RateLimiter.BYPASS);
        assertSuccessfulAcquire(20, limiter);
        limiter = RateLimiter.Factory.create(TokenBucket.class.getName(), RateLimiter.BYPASS, 10);
        assertSuccessfulAcquire(20, limiter);
        limiter = RateLimiter.Factory.create("bad.class", 1, 1); // fall back to bypass
        assertSuccessfulAcquire(20, limiter);
    }
    ...
    private long noRefillTime() {
        return System.nanoTime() + SECONDS.toNanos(10000);
    }
}
